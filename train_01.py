"""
train.py — Small transformer block trained with torch.profiler enabled.

This script is *intentionally* imperfect. It contains three common anti-patterns
that the analyzer should surface:

  1. A `.item()` call inside the training loop → forces a CPU-GPU sync, creating
     visible idle gaps on the GPU timeline.
  2. A small per-step batch size → kernels become launch-bound, many short ops.
  3. A non-fused optimizer (foreach=False) → lots of tiny kernels instead of one.

The profiler is configured with the schedule recipe: wait → warmup → active.
Only the "active" steps are recorded, which keeps the trace file small and
avoids the noise of the first few steps (cuDNN autotuning, allocator warmup).

Output: ./trace.json.gz  (Chrome trace format, gzip-compressed)
"""
...
def main():
    ...
    optim = torch.optim.AdamW(model.parameters(), lr=3e-4, foreach=False)
